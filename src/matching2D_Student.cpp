                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        num_matched_kpts = knn_matches.size();
...
{
    // select appropriate descriptor among BRIEF, ORB, FREAK, AKAZE, SIFT
    ...
    {
        cout << "Warning: Invalid Descriptor Type, runing default descriptor now..." << endl;
        int threshold = 30;        // FAST/AGAST detection threshold score.
        int octaves = 3;           // detection octaves (use 0 to do single scale)
        float patternScale = 1.0f; // apply this scale to the pattern used for sampling the neighbourhood of a keypoint.
...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    // cout <<cout << "Shi-Tomasi detection with n=" << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
